...
function angularInit(element, bootstrap) {
  var elements = [element],//检索加载angular的dom元素列表  
      appElement,//加载angular的dom元素  
      module,//加载的模块 
      names = ['ng:app', 'ng-app', 'x-ng-app', 'data-ng-app'],//检索angular加载的关键字 
      NG_APP_CLASS_REGEXP = /\sng[:\-]app(:\s*([\w\d_]+);?)?\s/;//提取angular加载模块的正则表达式
  ...
  forEach(names, function(name) {
    names[name] = true;
    append(document.getElementById(name));//对于ie7不支持element.querySelectorAll，需要设置id="ng-app"或names中的其他值
    name = name.replace(':', '\\:');
...
function bootstrap(element, modules) {
  var doBootstrap = function() {
    ...
    if (element.injector()) {//调用 JQLitePrototype.injector，这里不能重复注入
    ...
    //调用执行内部定义的function函数，参数'$rootScope', '$rootElement', '$compile', '$injector', '$animate'对应的函数为：
  ...
  //如果没有设置推迟加载，则直接调用doBootstrap
  if (window && !NG_DEFER_BOOTSTRAP.test(window.name)) {
    return doBootstrap();
  }

  window.name = window.name.replace(NG_DEFER_BOOTSTRAP, '');
  //如果设置了推迟加载，我们可以先给resumeBootstrap赋上引导函数，在需要的时候调用angular.resumeBootstrap即可。
  //这样我们可以在开始的时候设置window.name='NG_DEFER_BOOTSTRAP'; 在需要的时候再调用angular.resumeBootstrap方法
...
/**
 * 绑定jQuery对象，如果引入了jQuery，则使用jQuery提供的方法，并扩展了jQuery几个属性和方法
 * 同时对remove、empty和html进行了重写
 * 否则使用angular提供的jqLite，并赋值到angular.element上
 * 所以调用jQuery的方法就可以直接用angular.element
 */
function bindJQuery() {
 ...
 * @ngdoc interface
 * @name angular.Module
 * @description 创建angular默认模块  window.angular.module = ...
 *
 * Interface for configuring angular {@link angular.module modules}.
 */
function setupModuleLoader(window) {
  
  //定义module $injector 的错误处理变量
  ...
  return ensure(ensure(window, 'angular', Object), 'module', function() {
     ...
     */
    //加载module，name 为传入的module名称，例如
    //angularModule('ng', ['ngLocale'], ['$provide',function($provide){}]);
    return function module(name, requires, configFn) {
      //必须保证name不等于hasOwnProperty，否则抛出错误
      ...
      //该方法会为每个module绑定moduleInstance，包括requires、provider、factory、service、value、constant、animation、filter、controller、directive、config，
      //这样module就可以调用这些方法了（是通过$controllerProvider来解析的），setupModuleLoader封装的比较牛比
      return ensure(modules, name, function() {
        ...
        var moduleInstance = {
          // Private state
          _invokeQueue: invokeQueue,//对象队列，用来放入要执行的方法
...
function publishExternalAPI(angular){
  ...
  /******************************** 
  创建用于生成module的loader方法，即angularModule可以创建module，与开发中angular.module('moduleName',[],['depend',function(){depend}]);实现的功能是一样的
  这里之所以没有使用angular.module方式，主要是在setupModuleLoader(window)中第一次会创建angular.module，所以下面创建module都是利用angularModule来创建，单例的精髓
  在setupModuleLoader中有内部变量modules，每次创建一个module，都会以键值的方式赋值到modules上。modules是一对象，内部结构为（以ng为例，当然其他module也类似，如ngLocal）：
  modules
      ng
        Object { _invokeQueue=[1], _runBlocks=[0], requires=[1], 更多...}
      _invokeQueue
        [["$injector", "invoke", Object { 0=[2]}]]
      _runBlocks
        []
      name
        "ng"
      ...
      value
        function()
  modules也会包括自定义的module
  ********************************/  
  angularModule = setupModuleLoader(window);
  /******************************** 
  尝试挂载ngLocale模块，由于setupModuleLoader方法中requires是必须的，所以肯定会执行catch中的表达，不知道为什么要这样写？
  ********************************/  
  try {
    angularModule('ngLocale');
  } catch (e) {
    angularModule('ngLocale', []).provider('$locale', $LocaleProvider);//虽然调用了provider，但是不会执行该方法中的指定的函数，只是在内部变量invokeQueue中设置了函数队列
  }
  //配置ng模块，依赖于ngLocale，并依赖注入$provide，初始化系统默认相关指令
  angularModule('ng', ['ngLocale'], ['$provide',
    function ngModule($provide) {
      $provide.provider('$compile', $CompileProvider).//调用后返回的值同时拥有了$CompileProvider中的方法，如directive，所以可以直接调用directive
        directive({
...
function annotate(fn) {
  ...
  } else if (isArray(fn)) {//数组形式，比如：["$provide", ngModule($provide)]
    last = fn.length - 1;
...
function createInjector(modulesToLoad) {
  ...
  /**
   * 上面代码执行过程为createInternalInjector(providerCache, ...)返回包含以下属性的object赋给providerInjector
   * 然后又调用createInternalInjector(instanceCache, ...),调用完，就会形成如下的结构（这里会附图描述）
   */
   ...
   * @param {type} cache
   * @param {type} factory
   * @returns {_L28.createInjector.createInternalInjector.Anonym$24}ch
   */
...
function $CompileProvider($provide) {
  var hasDirectives = {},//angular中存在的指令
      Suffix = 'Directive',
  ...
  this.$get = [
    ...
    function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective,
      ...
      forEach($compileNodes, function(node, index){
        //是text node并且不是空字符，即非空白字符，需要包裹span
      ...
      switch(nodeType) {
        case 1: /* Element */
          ...
          for (var attr, name, nName, ngAttrName, value, nAttrs = node.attributes,
            ...
            attr = nAttrs[j];
            if (!msie || msie >= 8 || attr.specified) {//非ie7（包含）以下
              name = attr.name;
              // support ngAttr attribute binding
              ngAttrName = directiveNormalize(name);
              if (NG_ATTR_BINDING.test(ngAttrName)) {//对于ng_attr.*绑定的处理
