 ...
 * Possible options:
 *
 *	once:			will ensure the callback list can only be fired once (like a Deferred) 触发一次
 *
...
jQuery.extend({
	...
	Deferred: function( func ) {
		...
		// Keep pipe for back-compat 别名
		promise.pipe = promise.then;

		// Add list-specific methods
        // 处理三个异步方法
		jQuery.each( tuples, function( i, tuple ) {
			var list = tuple[ 2 ],//callbacks 对象 实例
				stateString = tuple[ 3 ];// 状态字符串

			// promise[ done | fail | progress ] = list.add
            // 把callbacks中的方法 add 赋到 promise 对象上，分别是 done | fail | progress
			promise[ tuple[1] ] = list.add;

			// Handle state
			if ( stateString ) {
                //添加回调函数，共三个，第一个是设置其状态的，后两个分别是禁用和锁定回调函数
				list.add(function() {
            ...
            // 为 resolve | reject | notify 定义三个函数，调用时会执行callbacks中的方法 fireWith
